...
const RULES = [
  ...
  // "review ingredients" added alongside "show"/"open" specifically for
  // the welcome overlay's "Review ingredients" button — its own label is
  // the single most natural phrase someone would say back to it, and
  // without this it silently fell through to null (not a regression from
  // the empty-arrays overlay fix elsewhere in this file; it was never
  // recognized, with or without steps/ingredients present).
  { action: "show-ingredients", test: /show me (the )?ingredients( list)?|show (the )?ingredients( list)?|open (the )?ingredients( list)?|review (the )?ingredients( list)?/ },
  // Bare "done" (anchored, so it can't match as a substring of an
  // unrelated phrase) is the voice equivalent of the ingredients panel's
  // own "Done" button (components/RecipePlayer.jsx) — harmless no-op if
  // said while the panel's already closed.
  { action: "hide-ingredients", test: /hide (the )?ingredients( list)?|close (the )?ingredients( list)?|dismiss (the )?ingredients( list)?|^done$/ },
...
// If two separate utterances land in the same SpeechRecognition result
// (documented latency/finalization behavior — see "Voice latency" in
// claude/dev-status-log.md; two closely-spaced utterances can merge into
// one transcript before onresult ever fires), a greedy capture on any of
// the four ingredient patterns below would swallow both into one
// reference phrase — e.g. "i need breadcrumbs i need eggs" captured
// whole would score both "breadcrumbs" and "eggs" as candidates and
// spuriously trigger the ambiguous-match popup instead of matching
// either one individually. This lookahead makes each pattern's capture
// stop right before whichever trigger phrase (this pattern's own, or a
// sibling's) starts the next utterance, so each command in a merged
// transcript still resolves independently regardless of timing.
const NEXT_INGREDIENT_TRIGGER =
  "(?=\\s+i\\s+need\\b|\\s+i\\s+have\\b|\\s+check\\s+off\\b|\\s+check\\b|\\s+got\\b|\\s+uncheck\\s+off\\b|\\s+uncheck\\b|\\s+unmark\\b|$)";

// "check off breadcrumbs" / "check breadcrumbs" / "got the eggs" — mark an
// ingredient checked by name.
const CHECK_INGREDIENT_PATTERN = new RegExp(
  `^(?:check off|check|got)\\s+(?:the\\s+)?(.+?)${NEXT_INGREDIENT_TRIGGER}`,
  "i"
);

// "I have breadcrumbs" / "I have some garlic" / "I have the eggs" — the
// voice equivalent of the "I have" radio, matched the same way "I need"
// is below rather than requiring the "check off"/"got" phrasing above.
const HAVE_INGREDIENT_PATTERN = new RegExp(
  `^i\\s+have\\s+(?:the\\s+)?(?:some\\s+)?(.+?)${NEXT_INGREDIENT_TRIGGER}`,
  "i"
);

// "uncheck breadcrumbs" / "uncheck off breadcrumbs" / "unmark eggs" — mark
// an ingredient unchecked by name.
const UNCHECK_INGREDIENT_PATTERN = new RegExp(
  `^(?:uncheck off|uncheck|unmark)\\s+(?:the\\s+)?(.+?)${NEXT_INGREDIENT_TRIGGER}`,
  "i"
);

// "I need breadcrumbs" / "I need some garlic" / "I need the eggs" — add
// an ingredient to the running "need to get" list by name.
const NEED_INGREDIENT_PATTERN = new RegExp(
  `^i\\s+need\\s+(?:the\\s+)?(?:some\\s+)?(.+?)${NEXT_INGREDIENT_TRIGGER}`,
  "i"
);
...
/**
 * Find every ingredient index tied for the MOST significant shared words
 * with the reference phrase — not just the single best, unlike
 * findBestMatchIndex above. Used for "I need <ingredient>," where more
 * than one real ingredient can genuinely share a word (e.g. "garlic"
 * against both "garlic" and "garlic powder") and the caller needs to
 * know when to ask rather than silently guess one. Returns [] if nothing
 * shares at least one meaningful word, or a single-element array for an
 * unambiguous match.
 */
function findMatchingIngredientIndices(reference, ingredients) {
  const refWords = significantWords(reference);
  if (!refWords.length || !ingredients?.length) return [];
 ...
 *    index } for "check off <ingredient>" / "I have <ingredient>" /
 *    "uncheck <ingredient>" style phrases, matched against the recipe's
 *    ingredients by name, or
 ...
 *  - { type: "need-ingredient", index } for "I need <ingredient>" when
 *    exactly one ingredient matches, or { type: "need-ingredient-ambiguous",
 *    indices } when more than one ties for the best fuzzy match — the
 *    caller should prompt to disambiguate rather than guess, or
...
export function matchVoiceCommand(transcript, steps = [], ingredients = []) {
  ...
  if (ingredients.length) {
    ...
    const haveMatch = text.match(HAVE_INGREDIENT_PATTERN);
    if (haveMatch) {
      const index = findBestMatchIndex(haveMatch[1], ingredients);
      if (index >= 0) return { type: "check-ingredient", index };
    }
    ...
    const needMatch = text.match(NEED_INGREDIENT_PATTERN);
    if (needMatch) {
      const indices = findMatchingIngredientIndices(needMatch[1], ingredients);
      if (indices.length === 1) return { type: "need-ingredient", index: indices[0] };
      if (indices.length > 1) return { type: "need-ingredient-ambiguous", indices };
    }
  }
