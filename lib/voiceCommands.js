...
const RULES = [
  ...
  // "let's get cooking" / "start cooking" / "skip" — dismiss the recipe
  // page's welcome overlay (components/RecipePlayer.jsx) without
  // reviewing ingredients first. Bare "skip" is anchored (^skip$) so it
  // can't swallow "skip ahead" above, which already means "next".
  // Reviewing ingredients from the same overlay reuses the existing
  // "show-ingredients" rule above rather than a separate command.
  { action: "dismiss-welcome", test: /^skip$|let'?s get cooking|start cooking/ },
];
